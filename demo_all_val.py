...
CACHE_PT  = f"{os.path.expandvars(STAGE_DIR)}/conch_cache_0927.pt"
CKPT_PT   = f"{os.path.expandvars(STAGE_DIR)}/projector_best_1016-3.pt"
...
print(f"Processing {len(val_idx)} validation samples...")
with torch.no_grad():
    for i in tqdm(val_idx, desc="Generating predictions"):
...
RES_OUT = f"{os.path.expandvars(STAGE_DIR)}/res_modelv1016-3_1112.json" # change output name here
